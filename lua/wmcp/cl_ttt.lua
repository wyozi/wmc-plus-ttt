...
hook.Add("WMCPMedialistRowRightClick", "WMCPAddDebugItem", function(menu, id, line, media)
	...
	tttsmpnl:AddOption("Toggle as Innocent end round music", function()
		RunConsoleCommand("wmcpttt_setendround", id, WIN_INNOCENT, is_innocent_music and "1" or "0")
	...
	tttsmpnl:AddOption("Toggle as Time limit reached end round music", function()
		RunConsoleCommand("wmcpttt_setendround", id, WIN_TIMELIMIT, is_timelimit_music and "1" or "0")
	end):SetIcon("icon16/" .. (is_timelimit_music and "cancel" or "add") .. ".png")

	tttsmpnl:AddOption("Toggle as Traitor end round music", function()
		RunConsoleCommand("wmcpttt_setendround", id, WIN_TRAITOR, is_traitor_music and "1" or "0")
	...
	do
		...
		for _,plr in pairs(player.GetAll()) do
			...
			plrsmpnl:AddOption("[".. plr:UserID() .. "] " .. plr:Nick(), function()
				RunConsoleCommand("wmcpttt_setplayer", id, steamid, checked and "1" or "0")
	...
	do
		...
		sidsmpnl:AddOption("Add by SteamID", function()
			Derma_StringRequest(
				...
				"",
				function(text)
					RunConsoleCommand("wmcpttt_setplayer", id, text, "0")
				end,
				function(text) end
			)
			...
			for k,v in pairs(media.ttt_opts) do
				...
				sidsmpnl:AddOption(k, function()
					Derma_Query("Are you sure you want to remove " .. k,
						"yay or nay",
						"yay", function()
							RunConsoleCommand("wmcpttt_setplayer", id, k, "1")
						end,
						"nay")
